...
/***/ function(module, exports, __webpack_require__) {
	...
	var logicEvents = __webpack_require__(8);
...
/***/ function(module, exports, __webpack_require__) {
	...
	var onIndexGames = function onIndexGames(event) {
	  event.preventDefault();
	  api.indexGames().done(ui.indexGameSuccess).fail(ui.failure);
	};

	var onShowGame = function onShowGame(event) {
	  event.preventDefault();
	  api.showGame().done(ui.success).fail(ui.failure);
	};

	// ALL 3 `GAME WIN` LOGIC BELOW
	// looks for horizontal win
	var horizontalWin = function horizontalWin(cells, id) {
	...
	var verticalWin = function verticalWin(cells, id) {
	...
	var diagonalWin = function diagonalWin(cells, id) {
	  var win = false;
	...
	var setGameStatus = function setGameStatus(messageHTML) {
	  $('#game-status').html(messageHTML);
	};

	var checkWin = function checkWin(cell) {
	  var id = $(cell).data('index');
	...
	var isTieGame = function isTieGame() {
	  if (turn === 8) {
	    return true;
	  }
	  return false;
	...
	var isGameOver = function isGameOver(cell) {
	  if (checkWin(cell)) {
	...
	var onCellClick = function onCellClick(event) {
	  ...
	  if (isValidMove(event.target)) {
	    ...
	    var cell = $(event.target).data('index');
	    var _over = isGameOver(event.target);
	    if (_over) {
	      // updateUserStats();

	    }
	    api.updateGame(cell, value, _over).done(ui.success).fail(ui.failure);
	    changePlayer();
	    turn++;
	  }
	};
...
/***/ function(module, exports, __webpack_require__) {
	...
	var updateGame = function updateGame(cell, player, over) {
	  var token = app.user.token;
